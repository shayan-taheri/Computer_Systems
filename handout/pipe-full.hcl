...
# Notice: In pipe hcl file, it is enough to add the "iddl" instruction and there in no need for adding "leave" or any other instruction. #

# Adding the "iddl" instruction has a good effect for optimizing the code because it compacts two instructions in one instruction. Therefore, There would be a fewer cycles to get the same result #
